...
test("it sorts a simple array", () => {
  assert.deepStrictEqual(mergeSort([9, 2, 10, 3, 5]), [2, 3, 5, 9, 10])
})

test("it sorts an array", () => {
  assert.deepStrictEqual(
    mergeSort([4, 9, 2, 4, 6, 1, 0, 23, 8, 6]),
...
test("it handles negative numbers", () => {
  assert.deepStrictEqual(mergeSort([4, -2, 0, -8, 1]), [-8, -2, 0, 1, 4])
})

test("it handles single numbers ", () => {
  assert.deepStrictEqual(mergeSort([4]), [4])
})
